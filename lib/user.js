...
User.prototype.setattack = function( json ) {
  ...
  if( unidadesAttaque >= jsonObject.warriors )
  {
      ...
      var que = this.attacks.sendWarriors( jsonObject.form_user , jsonObject.to_user , jsonObject.warriors , function( _result , _recursosRobados , _informe ) {
          ...
          /*Informes*/
          var inf = JSON.parse( _informe );
          console.log( "informe.text-------->" + inf.text );
          o.informes.addInforme( jsonObject.form_user , inf.text );

          //Habira que personalizar mejor los informes.
          o.informes.addInforme( jsonObject.to_user , inf.text );
...
function updateData( _this ) {
  ...
  var informes = JSON.stringify( _this.informes.getInformes( _this.userName ) );

  _this.socket_user.emit( 'updateData', '{ "informes":'+informes+'  ,"Maxpopulation":'+ _this.globals.max_poblacion +' , "population":'+ poblacion +' , "food":'+ valuefood +',"workers":'+ valueworkers +', "warriors":'+ valuewarriors +' , "que":'+ showTails +' , "attacks":'+ showAttackTails +' }' );
